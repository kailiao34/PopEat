...
public class FoodLis : MonoBehaviour
{
	...
	void GetResNames(List<string> resNames, List<Details> resDetails)
	{
		list = resNames;
		UISort();
	}
    ...
    public void UISort()
    {
		for (int i = 0; i < list.Count; i++) {
			...
			G1.transform.SetParent(FoodListUI_ScrollView_Content.transform);
			...
			foodListRectTransform[i].transform.SetParent(FoodListButtonParentobject.transform);
			if (i == 0) {
